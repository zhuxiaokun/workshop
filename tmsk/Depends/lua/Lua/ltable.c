/*
** $Id: ltable.c,v 1.1 2009/06/16 07:19:50 JIGUANG\seacat Exp $
** Lua tables (hash)
...
*/
static LuaNode *hashnum (const Table *t, lua_Number n) {
...
static void resize (lua_State *L, Table *t, int nasize, int nhsize) {
  ...
  if (nold != dummynode)
    luaM_freearray(L, nold, twoto(oldhsize), LuaNode);  /* free old array */
}
...
void luaH_free (lua_State *L, Table *t) {
  if (t->node != dummynode)
    luaM_freearray(L, t->node, sizenode(t), LuaNode);
...
*/
const TValue *luaH_getstr (Table *t, KLuaString *key) {
...
TValue *luaH_setstr (lua_State *L, Table *t, KLuaString *key) {
...
int luaH_isdummy (LuaNode *n) { return n == dummynode; }
