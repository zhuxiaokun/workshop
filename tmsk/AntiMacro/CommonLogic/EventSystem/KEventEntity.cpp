...
KEventEntity::KEventEntity()
{
	...
	RealTimerEntity::m_receiver = this;
}
...
bool KEventEntity::startRealTimer(DWORD name, Identity64 cookie,int mode, int val, int hour, int minute, int second, void* data)
{
	return RealTimerEntity::start(name, cookie, mode, val, hour, minute, second, data) != NULL;
}

void KEventEntity::stopRealTimer(DWORD name, Identity64 cookie)
{
	RealTimerEntity::stop(name, cookie);
}

void KEventEntity::clearRealTimer()
{
	RealTimerEntity::clear();
}
...
bool KEventEntity::onRealTimer(DWORD name, Identity64 cookie, const void* data)
{
	return true;
}
...
bool KEventEntity_luaMaster::onRealTimer(DWORD name, Identity64 cookie, const void* data)
{
	if(this->luaHandleRealTimer(name, cookie, data))
		return true;
	if(m_pAssist)
		return m_pAssist->onRealTimer(name, cookie, data);
	return false;
}
