...
bool KAttrParser::Parse(const char* attrs, char delimeter)
{
	...
	char* src = buf->WriteData(attrs, strlen(attrs)+1);
...
bool KAttrParser::Set(const char* name, const char* val)
{
	...
	if(pPair)
	{
		...
		{
			KBuffer64k* buf = this->getBuffer();
			pPair->val = buf->WriteData(val, strlen(val)+1);
	...
	else
	{
		...
		KAttrPair pair;
		pair.name = buf->WriteData(name, strlen(name)+1);
		pair.val = buf->WriteData(val, strlen(val)+1);
...
int KAttrParser::ToString(char* buf, int len) const
{
	int pos = 0;
	int c = this->Size();
	for(int i=0; i<c; i++)
