...
void KLogFileReader::breathe(int interval)
{
	...
	while(1)
	{
		...
		totalLineCount += m_frameLineCount;
		if(!m_frameLineCount || totalLineCount > 10000)
			break;
	}
}
...
bool KLogFileReader::onTimer(DWORD name, Identity64 cookie, const void* data)
{
	...
	case AppTimer_WaitDispatcher:
		return this->onTimer_waitDispatcher(name, cookie, data);
	}
	return true;
}
...
bool KLogFileReader::onTimer_readLogFile(DWORD name, Identity64 cookie, const void* data)
{
	if(!m_readBuf.ReadAvail())
	{
		if(!this->_readToBuffer())
		{
			...
			{
				this->startTimer(AppTimer_ReadLogFile, 0, 5.0f, NULL);
			}
			return true;
		}
	}
	...
	while(1)
	{
		if(!g_pApp->isDispatcherReady())
		{
			this->startTimer(AppTimer_WaitDispatcher, 0, 2.f, NULL);
			return true;
		}
...
bool KLogFileReader::onTimer_waitDispatcher(DWORD name, Identity64 cookie, const void* data)
{
	if(g_pApp->isDispatcherReady())
	{
		this->startTimer(AppTimer_ReadLogFile, 0, 0.0f, NULL);
