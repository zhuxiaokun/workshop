...
public:
	bool scanIpList();
	bool scanPatternIpList(const char* ip_pattern);
	KIp* loadIp(const char* ip);
...
public:
	BeginDefLuaClassNoneGc(KIpAnalysis);
		DefMemberFunc(reset);
		DefMemberFunc(scanIpList);
		DefMemberFunc(scanPatternIpList);
		DefMemberFunc(loadIp);
		DefMemberFunc(foreachIpName);
		DefMemberFunc(foreachIpObject);
		//DefMemberFunc(foreachIpServer);
