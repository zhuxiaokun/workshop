...
class KActiveServerSet : public hscan_visitor
{
public:
	typedef JG_C::KDynSortedArray<ccstr,KccstrCmp,128> ServerArray;
...
public:
	bool loadFromRedis(KRedis& redis, time_t start, const char* redisAcctKey);
...
public:
	time_t m_startTime;
	KActiveServerSet m_activeServerSet;
...
public:
	...
	bool load(KRedis& redis, int hours, const char* acctName);
...
public:
	...
	int serverCount() const { return m_activeServerSet.size(); }
	const char* serverNameAt(int i) const { return m_activeServerSet.at(i); }
	KServer* serverAt(int i) { return m_serverMap[serverNameAt(i)]; }
...
public:
	BeginDefLuaClassNoneGc(KAccount);
		...
		DefMemberFunc(serverNameAt);
		DefMemberFunc(serverAt);
