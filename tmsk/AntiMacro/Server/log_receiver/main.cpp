...
int main(int argc, char* argv[])
{
	...
	//////
	AsyncDailyFileLogger* logger = new AsyncDailyFileLogger("./log/log_receiver.log");
	logger->SetLogLevel(LOG_ALL);
	setGlobalLogger(logger);

	Log(LOG_ERROR|LOG_CONSOLE, "======================================");
	Log(LOG_ERROR|LOG_CONSOLE, "|        log_receiver    server       |");
	Log(LOG_ERROR|LOG_CONSOLE, "======================================");
	...
	const char* cfgFilePath = "../settings/log_receiver/configure.lua";
	if(argc > 1)
	{
		cfgFilePath = argv[1];
