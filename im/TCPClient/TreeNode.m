...
#import "TreeNode.h"
//#define IPAD_POS_X 64.0f
//#define IPAD_POS_Y 32.0f
//
//#define IPAD_RATE  2
...
- (void) dumpAtIndent: (int) indent into:(NSMutableString *) outstring
{
    ...
    {
        /*------------------------------------------------------------------------------*
        NSDictionary *dic = [node atributes];
        NSString *leaf;
        if (dic) {
            if ([[dic objectForKey:@"key"] isEqualToString:@"NSFrameSize"]) {
                leaf = [node leafvalue];
                if (leaf) {
                    node.leafvalue = @"{768, 1024}";
                }
            }
        ...
        if ([leaf hasPrefix:@"{{"] && [leaf hasSuffix:@"}}"]) {
            leaf = [leaf stringByReplacingOccurrencesOfString:@"{" withString:@""];
            leaf = [leaf stringByReplacingOccurrencesOfString:@"}" withString:@""];
            NSArray *array = [leaf componentsSeparatedByString:@","];
            
            float ipad_PosX = [[array objectAtIndex:0] floatValue] + IPAD_POS_X;
            float ipad_PosY = [[array objectAtIndex:1] floatValue] + IPAD_POS_Y;
            
            float ipad_Width = [[array objectAtIndex:2] floatValue] * IPAD_RATE;
            
            float ipad_Height = [[array objectAtIndex:3] floatValue] * IPAD_RATE;
            
            NSString *newFrame = [NSString stringWithFormat:@"{{%f,%f},{%f,%f}}", ipad_PosX, ipad_PosY, ipad_Width, ipad_Height];
            
            node.leafvalue = newFrame;
            
        }
        
//        else if ([leaf hasPrefix:@"{"] && [leaf hasSuffix:@"}"]) {
//            
//            leaf = [leaf stringByReplacingOccurrencesOfString:@"{" withString:@""];
//            leaf = [leaf stringByReplacingOccurrencesOfString:@"}" withString:@""];
//            NSArray *array = [leaf componentsSeparatedByString:@","];
//            
//            float ipad_PosX = [[array objectAtIndex:0] floatValue] * IPAD_RATE;
//            float ipad_PosY = [[array objectAtIndex:1] floatValue] * IPAD_RATE;
//            
//            NSString *newPosition = [NSString stringWithFormat:@"{%f,%f}", ipad_PosX, ipad_PosY];
//            node.leafvalue = newPosition;
//        }
