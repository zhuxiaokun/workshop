 ...
 */
+ (void)showView:(UIImage*)_view animateType:(AnimateType)_aType finalRect:(CGRect)_fRect completion:(void(^)(BOOL finished))completion{
    //初始化页面数组
    ...
    info.displayView =[[UIImageView alloc]initWithImage:[Photo scaleImage:_view toWidth:_fRect.size.width toHeight:_fRect.size.height]];
    info.aType = _aType;
...
/**
 ...
 @param _fRect 最终位置
 */
+ (void)showView:(UIImage *)_view animateType:(AnimateType)_aType finalRect:(CGRect)_fRect{
