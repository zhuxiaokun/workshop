...
#import "cCustomMethod.h"

@implementation cCustomMethod
+(NSString *)escapedString:(NSString *)oldString
{
    NSString *escapedString_lt=[oldString stringByReplacingOccurrencesOfRegex:@"<" withString:@"&lt"];
...
#pragma mark - 正则匹配电话话吗 ，网址连接 ，Email地址
...

{
    ...
    NSArray *array_http = [text componentsSeparatedByRegex:regex_http];
...
+(NSMutableArray *)addPhoneNumArr:(NSString *)text
{
    ...
    NSArray *array_phonenum=[text componentsSeparatedByRegex:regex_phonenum];
...
+ (NSMutableArray *)addEmailArr:(NSString *)text
{
    ...
    NSArray *array_email = [text componentsSeparatedByRegex:regex_email];
    NSMutableArray *emailarray=[NSMutableArray arrayWithArray:array_email];
    return emailarray;
}
